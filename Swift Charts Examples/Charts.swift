...
struct ContentView: View {
    ...
    var body: some View {
        NavigationSplitView {
                ...
                ForEach(displayedCategories) { category in
                    Section {
                        ForEach(items) { item in
                            let buffer = item.title + " \n" + Date().formatted()
                            
                            NavigationLink(buffer){
                                //preview(chart: chart)
                                HeartBeat(isOverview: false, data1: item.dataPoints, title: item.title)
                            }
                        }
                    } header: {
                        Text("")
            ...
            .modelContainer(DataController.populateContainer)
            #if os(macOS)
            .listStyle(.bordered)
            #else
